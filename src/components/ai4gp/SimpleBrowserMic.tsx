...
export const SimpleBrowserMic = forwardRef<SimpleBrowserMicRef, SimpleBrowserMicProps>(({
  ...
  const getButtonContent = () => {
    if (status === 'connecting...') {
      return <Loader2 className="w-56 h-56 animate-spin" />;
    } else if (isRecording) {
      return <MicOff className="w-56 h-56" />;
    } else {
      return <Mic className="w-56 h-56" />;
    }
  };
  ...
  return (
    <div className={cn('flex items-center gap-2', className)}>
      <Button
        ...
        size="sm"
        className={cn(
          "h-60 w-60 p-0 transition-all duration-200",
          isRecording 
