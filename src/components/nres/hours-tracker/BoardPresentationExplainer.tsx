...
<p style={{fontSize:14,color:T.sub,lineHeight:1.8,margin:0}}>Every claim under NRES must demonstrate that the funded activity is <strong style={{color:T.navy}}>genuinely additional</strong> — new work that would not have occurred without the programme. No additionality, no payment.</p>
</div></div>
...
<div style={{fontSize:13,color:T.sub,marginTop:6,lineHeight:1.6}}>Claiming for existing staff whose current SDA time is formally attributed to the NRES programme</div>
</div>
<div style={{padding:"20px 24px",display:"grid",gap:16}}>
...
{icon:"🔍",title:"Approval path",body:"Draft → Submitted → Verified by Malcolm/Amanda → Approved by SNO. Extra verification because buyback evidence requires closer scrutiny."}
].map((item,i)=>
<div key={i} style={{display:"flex",gap:12,alignItems:"flex-start"}}>
...
{[{n:"1",l:"Draft",s:"Practice builds",c:T.muted},{n:"2",l:"Submitted",s:"4 evidence uploaded",c:T.r1},{n:"3",l:"Verified",s:"Malcolm / Amanda",c:"#D97706",a:true},{n:"4",l:"Approved",s:"SNO (Dr Gray)",c:T.emerald},{n:"5",l:"Paid",s:"PML Finance",c:"#7C3AED"}].map((s,i)=>
...
{who:"Malcolm / Amanda",icon:"🔍",tasks:["Verify buyback evidence (Route 2)","Check all 4 docs present & correct","Return with notes if incomplete","Pass verified claims to SNO","Track LTC evidence obligations"],c:"#D97706"},
...
{n:"3",t:"Verified by NRES team",d:"Malcolm/Amanda check all 4 docs — clinician matches, period correct, LTC is demonstrably new."},
