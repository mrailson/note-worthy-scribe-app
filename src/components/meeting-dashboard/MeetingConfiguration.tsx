...
interface MeetingConfig {
  ...
  agendaFiles: Array<{ name: string; content: string }>;
}
...
interface MeetingConfigurationProps {
  ...
  onLoadTemplate?: (config: MeetingConfig) => void;
}

export const MeetingConfiguration: React.FC<MeetingConfigurationProps> = ({
  config,
  onConfigChange,
  onSaveTemplate,
  onLoadTemplate
  ...
  const handleAgendaUpload = async (files: FileList) => {
    try {
      ...
      const newAgenda = config.agenda + combinedAgenda;
      
      onConfigChange({ 
        ...config,
        agenda: newAgenda,
        agendaFiles: [...(config.agendaFiles || []), ...processedFiles]
      });
  ...
  const saveAsTemplate = () => {
    const templateName = prompt('Enter a name for this meeting template:');
    ...
    <div className="space-y-6">
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <div className="flex justify-between text-xs text-muted-foreground">
              ...
              <span>{config.title.length}/100</span>
            </div>
          </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Attendees ({config.attendees.length})
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex items-center gap-2">
          ...
          {config.attendees.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {config.attendees.slice(0, 8).map((attendee, index) => (
                <Badge key={index} variant="secondary">
                  {attendee.name}
                  {attendee.title && ` (${attendee.title})`}
                </Badge>
              ))}
              {config.attendees.length > 8 && (
                <Badge variant="outline">
                  +{config.attendees.length - 8} more
                </Badge>
              )}
            </div>
          )}
          ...
          {showAttendeeManager && (
            <div className="border rounded-lg p-4">
              <AttendeeManager 
                onAttendeesChange={(attendees) => onConfigChange({ ...config, attendees })}
      ...
      <Card className="flex-1">
        ...
        <CardContent className="space-y-4 h-full">
          ...
          {config.agendaFiles?.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {config.agendaFiles.map((file, index) => (
          ...
          <div className="space-y-2 flex-1">
            ...
            <Textarea
              id="agenda"
              value={config.agenda}
              onChange={(e) => onConfigChange({ ...config, agenda: e.target.value })}
      ...
      {(config.title || config.attendees.length > 0 || config.agenda) && (
        <Card className="border-primary/20 bg-primary/5">
          ...
          <CardContent className="text-xs text-muted-foreground space-y-2">
            ...
            {config.attendees.length > 0 && (
              <div><strong>Attendees:</strong> {config.attendees.length} participants</div>
            )}
            {config.agenda && (
              <div><strong>Agenda:</strong> {config.agenda.length} characters of context</div>
            )}
            <div className="text-success text-xs mt-2">
              ✓ This configuration will be applied to recordings and imports
            </div>
          </CardContent>
        </Card>
      )}
