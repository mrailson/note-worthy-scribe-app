...
const SpecToggle: React.FC<{ checked: boolean; onChange: (v: boolean) => void; id?: string }> = ({ checked, onChange, id }) => (
  <button
    id={id}
    type="button"
    role="switch"
...
export const DocumentSettingsModal: React.FC<DocumentSettingsModalProps> = ({ isOpen, onClose, onApply }) => {
  ...
  const handleDrop = useCallback((e: React.DragEvent) => {
    ...
    if (file && /\.(png|jpg|jpeg|svg|webp)$/i.test(file.name)) setNewFile(file);
  }, []);
    ...
    >
        ...
        <div style={{
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '16px 18px' }}>
            ...
            <div style={{ background: '#f9fafb', borderRadius: 8, padding: 14, marginBottom: 12 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                <div>
                  ...
                  {activeLogo && (
                    <p style={{ fontSize: 12, color: '#9ca3af', marginTop: 1 }}>{truncateName(activeLogo.name)}</p>
                  )}
                </div>
                ...
                <>
                  {logos.length > 0 && (
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, marginBottom: 8 }}>
                      {logos.map(logo => {
                        const isSelected = logo.is_active;
                        const badge = TYPE_BADGE[logo.type] || TYPE_BADGE.practice;
                        return (
                          <button
                            key={logo.id}
                            type="button"
                            ...
                            style={{
                              ...
                              border: isSelected ? '2px solid #003087' : '0.5px solid #e5e7eb',
                          ...
                          >
                            {isSelected && (
                              <span style={{
                                ...
                                width: 16, height: 16, borderRadius: '50%', background: '#003087',
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                    ...
                    <select
                      value={localSettings.logo_position}
                      onChange={e => updateLocal('logo_position', e.target.value)}
                      style={{ fontSize: 12, padding: '3px 8px', borderRadius: 6, border: '0.5px solid #e5e7eb', background: '#fff', cursor: 'pointer' }}
                    >
                    ...
                    <div style={{ border: '0.5px solid #e5e7eb', borderRadius: 8, padding: 12, background: '#fff' }}>
                      <input
                        ...
                        style={{
                          ...
                          border: '0.5px solid #e5e7eb', marginBottom: 8, boxSizing: 'border-box',
                          outline: 'none',
                        }}
                      />
                      <select
                        ...
                        style={{
                          width: '100%', fontSize: 12, padding: '6px 8px', borderRadius: 6,
                          border: '0.5px solid #e5e7eb', marginBottom: 8, cursor: 'pointer',
                        }}
                      >
                      ...
                      <div
                        ...
                        style={{
                          border: '1.5px dashed #d1d5db', borderRadius: 8, padding: 10,
                      ...
                      <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                        <button
                          type="button"
                          onClick={() => { setShowAddForm(false); setNewName(''); setNewFile(null); }}
                          style={{ fontSize: 12, padding: '5px 12px', borderRadius: 6, border: '0.5px solid #e5e7eb', background: '#fff', cursor: 'pointer' }}
                        >Cancel</button>
                        <button
                          type="button"
                          ...
                          style={{
                            ...
                            border: 'none', background: '#003087', color: '#fff', cursor: 'pointer',
                            opacity: isSavingLogo ? 0.6 : 1,
                          }}
                        >{isSavingLogo ? 'Saving…' : 'Save logo'}</button>
                      </div>
                    </div>
                  )}
                </>
            ...
            {/* Footer */}
            <div style={{
              background: '#f9fafb', borderRadius: 8, padding: '12px 14px', marginBottom: 20,
              ...
              {SECTION_META.map((sec, idx) => (
                <div
                  ...
                  style={{
                    display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                    padding: '12px 14px',
                    borderBottom: idx < SECTION_META.length - 1 ? '0.5px solid #e5e7eb' : 'none',
                  }}
                >
                  <div>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                {pills.length === 0 && (
                  <span style={{ fontSize: 11, color: '#9ca3af', fontStyle: 'italic' }}>No sections selected</span>
                )}
                {pills.map(p => {
                  const c = PILL_COLOURS[p.pillKey] || PILL_COLOURS.logo;
                  return (
                    <span key={p.pillKey} style={{ fontSize: 11, fontWeight: 500, padding: '2px 8px', borderRadius: 20, background: c.bg, color: c.fg }}>
                      {p.label}
                    </span>
                  );
                })}
          ...
          {/* Footer buttons */}
          <div style={{
            borderTop: '0.5px solid #e5e7eb', padding: '12px 18px',
            display: 'flex', justifyContent: 'flex-end', gap: 8,
          }}>
            <button
              type="button"
              onClick={onClose}
              style={{ fontSize: 13, padding: '9px 16px', borderRadius: 8, border: '0.5px solid #e5e7eb', background: '#fff', cursor: 'pointer' }}
            >Cancel</button>
            <button
              type="button"
              onClick={handleApply}
              style={{ fontSize: 13, padding: '9px 16px', borderRadius: 8, border: 'none', background: '#003087', color: '#fff', cursor: 'pointer' }}
            >Apply</button>
          </div>
        </div>
      </div>
