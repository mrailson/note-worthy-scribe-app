                ...
                {docSettings.footer_on && (
                  <div className="mt-8 pt-4 border-t flex items-center justify-between text-xs" style={{ color: COLORS.lightGrey }}>
                    <div>
                      ...
                      {(() => {
                        const llmUsed = localStorage.getItem(`meeting-llm-used-${meetingId}`);
                        if (llmUsed && llmUsed !== 'gemini-3-flash') {
                          const label = llmUsed === 'claude-sonnet-4-6' ? 'Claude Sonnet 4.6' :
                            llmUsed === 'claude-opus-4-6' ? 'Claude Opus 4.6' : llmUsed;
                          return <p className="mt-0.5 text-[10px]" style={{ color: COLORS.lightGrey }}>LLM: {label}</p>;
                        }
                        return null;
                      })()}
                    </div>
