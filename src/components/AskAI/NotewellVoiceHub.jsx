...
const VOICE_SERVICES = [
  {
    ...
    icon: "🏘️",
    agentEnvKey: "VITE_NRES_ELEVENLABS_AGENT_ID",
    accentColor: "#005EB8",
  ...
  },
  {
    ...
    icon: "🌐",
    agentEnvKey: "VITE_TRANSLATION_ELEVENLABS_AGENT_ID",
    accentColor: "#009639",
  ...
  },
  {
    ...
    icon: "🩺",
    agentEnvKey: "VITE_TRIAGE_ELEVENLABS_AGENT_ID",
    accentColor: "#AE2573",
  ...
  },
  {
    ...
    icon: "💙",
    agentEnvKey: "VITE_AGEWELL_ELEVENLABS_AGENT_ID",
    accentColor: "#41B6E6",
...
export default function NotewellVoiceHub() {
  ...
  const startSession = useCallback(
    async (service) => {
      ...
      try {
        await prewarmAudio();
        // REAL SDK — uncomment when @elevenlabs/react is installed:
        // const agentId = import.meta.env[service.agentEnvKey];
        // await conversation.startSession({ agentId });
        // Simulation (remove when real SDK wired):
        setTimeout(() => {
          setStatus(service.id, "active");
          addMessage(service.id, "agent", `Hello, I'm the ${service.name}. How can I help you today?`);
        }, 1200);
      } catch (err) {
        console.error(err);
  ...
  const endSession = useCallback((id) => {
    setStatus(id, "ended");
