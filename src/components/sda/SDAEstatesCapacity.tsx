...
import { CheckCircle2, MapPin, Sun, Snowflake, Building2, Clock, Users, Calendar, LayoutGrid, CalendarDays, CalendarRange, ArrowUpDown, ArrowUp, ArrowDown, ChevronUp, ChevronDown, Info, Pencil, Save, X, ZoomIn, ZoomOut } from "lucide-react";
      ...
      <Dialog open={showMapModal} onOpenChange={(open) => { setShowMapModal(open); if (!open) setMapZoom(100); }}>
        <DialogContent className="!max-w-none !w-screen !h-screen !max-h-screen !translate-x-[-50%] !translate-y-[-50%] !rounded-none p-0 overflow-auto border-0 bg-[#0e1a2e] mx-0 my-0">
          ...
          <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[100] flex items-center gap-3 bg-[#0e1a2e]/90 backdrop-blur-sm border border-white/10 rounded-full px-4 py-2.5 shadow-lg">
            <button onClick={() => setMapZoom(Math.max(50, mapZoom - 10))} className="text-white/70 hover:text-white transition-colors" aria-label="Zoom out">
              <ZoomOut className="w-4 h-4" />
            </button>
            <Slider value={[mapZoom]} onValueChange={(v) => setMapZoom(v[0])} min={50} max={150} step={5} className="w-40" />
            <button onClick={() => setMapZoom(Math.min(150, mapZoom + 10))} className="text-white/70 hover:text-white transition-colors" aria-label="Zoom in">
