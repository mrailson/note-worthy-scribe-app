...
interface ProgrammeAuditLogDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  entries: AuditEntry[];
}

export const ProgrammeAuditLogDialog: React.FC<ProgrammeAuditLogDialogProps> = ({
  open,
  onOpenChange,
  entries,
}) => {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[680px] max-h-[calc(100vh-8rem)] bg-background">
        <DialogHeader className="px-8 sm:px-10 pt-2">
          <DialogTitle>Programme Plan – Audit Log</DialogTitle>
          <DialogDescription>
            All changes made to the programme plan are recorded below.
          </DialogDescription>
        </DialogHeader>
