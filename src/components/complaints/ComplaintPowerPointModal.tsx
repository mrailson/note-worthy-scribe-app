...
export const ComplaintPowerPointModal: React.FC<ComplaintPowerPointModalProps> = ({
  ...
  const estimatedDuration = DEFAULT_SLIDE_COUNT * 15;
  ...
  useEffect(() => {
    if (isOpen && !hasStarted && !isComplete) {
      ...
      generatePowerPoint(complaintData, DEFAULT_SLIDE_COUNT).then((result) => {
        if (result.success) {
          setIsComplete(true);
  ...
  }, [isOpen, hasStarted, isComplete, generatePowerPoint, complaintData, onClose, estimatedDuration]);
        ...
        <div className="flex items-center justify-between p-4 border-b bg-gradient-to-r from-primary/10 to-accent/10">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-foreground">
              {isComplete ? 'Complete!' : hasFailed ? 'Generation Failed' : 'Creating Staff Training PowerPoint'}
            </span>
          </div>
            ...
            {isComplete ? (
              ...
              <>
                ...
                <p className="text-xs text-muted-foreground/70">
                  Creating {DEFAULT_SLIDE_COUNT}-slide anonymised training presentation
                </p>
              </>
            )}
          </div>
