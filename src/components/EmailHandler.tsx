...
export const EmailHandler = ({ resetTrigger }: EmailHandlerProps = {}) => {
  ...
  const convertContentToStyledHTML = (text: string): string => {
    ...
    return `${emailCSS}<div class="message-container">${text.replace(/\n/g, '<br>')}<div class="signature">Generated by NHS GP Translation Service</div></div>`;
  };

  const sendEmailAndDownloadProof = async () => {
    ...
    // Start with downloading proof document
    ...
    try {
      ...
      const emailData = {
        ...
        from_name: 'NHS GP Translation Service',
              ...
              {qualityAssessment && emailReply && (
                <div className="space-y-4">
                  ...
                  <div className="flex gap-2">
                    <Button
                      onClick={sendEmailAndDownloadProof}
                      disabled={(isSending || isDownloadingProof) || qualityAssessment.overallSafety === 'unsafe'}
                    >
                      {isDownloadingProof ? (
                        <>
                          <Loader2 className="w-4 h-4 mr-2 animate-spin" />
