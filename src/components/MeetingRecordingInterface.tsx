...
export const MeetingRecordingInterface: React.FC<MeetingRecordingInterfaceProps> = ({ onClose }) => {
  ...
  const speechTranscriberRef = useRef<BrowserSpeechTranscriber | null>(null);
  ...
  const initializeRealtimeTranscription = () => {
    console.log('🎙️ Initializing real-time transcription...');
    
    speechTranscriberRef.current = new BrowserSpeechTranscriber(
      (data: TranscriptData) => {
        console.log('📝 Real-time transcript received:', data);
        setRealtimeTranscripts(prev => [...prev.slice(-10), data]); // Keep last 10 entries
        
        // Update live transcript with cumulative text
        if (data.is_final) {
          setLiveTranscript(prev => {
            const newTranscript = prev + ' ' + data.text;
            console.log('✅ Final transcript added to live transcript');
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      initializeRealtimeTranscription();
      if (speechTranscriberRef.current) {
        await speechTranscriberRef.current.startTranscription();
  ...
  const stopRecording = () => {
    if (mediaRecorderRef.current?.state === 'recording') {
      ...
      if (speechTranscriberRef.current) {
        speechTranscriberRef.current.stopTranscription();
  ...
  const processRecordingBasic = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn('⚠️ Transcription failed, using live transcript as fallback:', transcriptError);
        
        // Use live transcript if available, otherwise basic note
        const finalTranscript = liveTranscript.trim() || 
          `Meeting recorded from ${new Date(Date.now() - recordingTime * 1000).toLocaleTimeString()} to ${new Date().toLocaleTimeString()}. Duration: ${Math.floor(recordingTime / 60)} minutes ${recordingTime % 60} seconds.`;
            ...
            <TabsContent value="transcript" className="space-y-4">
              <div className="text-center">
                ...
                <p className="text-sm text-muted-foreground">
                  {isRecording ? 'Transcribing in real-time...' : 'Start recording to see live transcript'}
                </p>
              </div>
              ...
              <div className="border rounded-lg p-4 min-h-[300px] max-h-[400px] overflow-y-auto bg-muted/20">
                ...
                ) : liveTranscript || realtimeTranscripts.length > 0 ? (
                  <div className="space-y-2">
                    {realtimeTranscripts.map((item, index) => (
                      <div key={index} className="text-sm">
                        <span className={`${item.is_final ? 'text-foreground' : 'text-muted-foreground italic'}`}>
                          {item.text}
                        </span>
                        {item.is_final && index < realtimeTranscripts.length - 1 && <br />}
                      </div>
                    ))}
                    {liveTranscript && (
