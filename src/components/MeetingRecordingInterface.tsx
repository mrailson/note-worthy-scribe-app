...
export const MeetingRecordingInterface: React.FC<MeetingRecordingInterfaceProps> = ({ onClose }) => {
  ...
  const startRecording = async () => {
    try {
      ...
      console.log('✅ Media access granted, creating MediaRecorder...');
      ...
      mediaRecorder.ondataavailable = async (event) => {
        if (event.data.size > 0) {
          console.log(`📦 Storing audio chunk ${chunkNumber} (${event.data.size} bytes)`);
          await storeAudioChunk(event.data, chunkNumber++, meeting.id);
        }
      };

      mediaRecorder.onstop = () => {
        console.log('⏹️ Recording stopped, cleaning up...');
