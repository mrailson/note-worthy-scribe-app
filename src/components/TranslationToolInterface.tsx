...
export const TranslationToolInterface = () => {
  ...
  // Email current translation to logged-in user
  const handleEmailToMe = async () => {
    if (!currentTranslation) {
      toast.error('No translation available to email');
      return;
    }
    ...
    try {
      ...
      // Create formatted content for email
      const emailContent = `
        <div class="translation-section">
          <h2>Live Translation Session</h2>
          <p><strong>Date:</strong> ${currentTranslation.timestamp.toLocaleDateString()}</p>
          <p><strong>Time:</strong> ${currentTranslation.timestamp.toLocaleTimeString()}</p>
        </div>
        
        <div class="translation-section">
          <h3>Original Text (English - GP):</h3>
          <p>${currentTranslation.englishText}</p>
        </div>
        
        <div class="translation-section">
          <h3>Translation (${currentTranslation.targetLanguage.charAt(0).toUpperCase() + currentTranslation.targetLanguage.slice(1)} - Patient):</h3>
          <p>${currentTranslation.translatedText}</p>
        </div>
        
        <div class="translation-section">
          <p><em>This translation has been verified as safe and accurate by Notewell AI Translation Service.</em></p>
        </div>
      `;

      const emailData = {
        to_email: user.email,
        subject: 'Translation Session - GP Practice Record',
        message: convertContentToStyledHTML(emailContent),
      ...
      toast.success(`Translation emailed successfully to ${user.email}`);
    } catch (error) {
  ...
  // Email patient-friendly translation (theoretical patient email)
  const handleEmailToPatient = async () => {
    ...
    try {
      ...
      // Create patient-friendly content (sent to GP's email for forwarding)
      const patientContent = `
        <div class="translation-section">
          <h2>Patient Translation Copy</h2>
          <p><strong>Date:</strong> ${currentTranslation.timestamp.toLocaleDateString()}</p>
          <p><strong>Time:</strong> ${currentTranslation.timestamp.toLocaleTimeString()}</p>
        </div>
        
        <div class="translation-section">
          <h3>Your message in ${currentTranslation.targetLanguage.charAt(0).toUpperCase() + currentTranslation.targetLanguage.slice(1)}:</h3>
          <p style="font-size: 1.2em; font-weight: 500;">${currentTranslation.translatedText}</p>
        </div>
        
        <div class="translation-section">
          <h3>GP's response in English:</h3>
          <p style="font-size: 1.2em; font-weight: 500;">${currentTranslation.englishText}</p>
        </div>
        
        <div class="translation-section">
          <p><em>This is a patient copy of the translation session for your records. Please forward to the patient as appropriate.</em></p>
        </div>
      `;

      const emailData = {
        to_email: user.email, // Sent to GP for forwarding to patient
        subject: 'Patient Copy - Translation Session',
        message: convertContentToStyledHTML(patientContent),
      ...
      toast.success(`Patient copy emailed to ${user.email} for forwarding`);
    } catch (error) {
                     ...
                     <DropdownMenuTrigger asChild>
                      ...
                      <DropdownMenuContent align="center" className="w-64 bg-background border shadow-lg z-[110]">
                        ...
                        <DropdownMenuItem 
                          ...
                          disabled={!currentTranslation || isEmailingSelf}
                          className="flex items-center gap-2 cursor-pointer py-3"
                        >
                          <Mail className="w-4 h-4" />
                          {isEmailingSelf ? 'Emailing to Me...' : 'Email to Me'}
                        </DropdownMenuItem>
                        <DropdownMenuItem 
                          onClick={handleEmailToPatient}
                          disabled={!currentTranslation || isEmailingPatient}
                          className="flex items-center gap-2 cursor-pointer py-3"
                        >
                          <Mail className="w-4 h-4" />
                          {isEmailingPatient ? 'Emailing Patient Copy...' : 'Email to Patient'}
                        </DropdownMenuItem>
                      </DropdownMenuContent>
                   </DropdownMenu>
                </div>
