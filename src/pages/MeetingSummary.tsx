              ...
              <div className={practiceData?.logo_url ? '' : 'w-full'}>
                ...
                <div className="mt-2">
                  <p className="text-xs sm:text-sm text-muted-foreground">
                    <span className="font-medium">Meeting:</span> {meetingData?.title || 'General Meeting'} | <span className="font-medium">Type:</span> {meetingSettings.meetingType.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase())} | <span className="font-medium">Date & Time:</span> {new Date(meetingData?.startTime || new Date()).toLocaleDateString('en-GB')} at {new Date(meetingData?.startTime || new Date()).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                  </p>
                </div>
              </div>
            </div>
