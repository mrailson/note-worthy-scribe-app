...
Generated by Notewell AI Meeting Notes Service`;
  ...
  const generateAIMeetingMinutes = async (customDetailLevel?: string) => {
    ...
    const transcript = meetingData?.transcript || transcriptData.map(t => t.content).join(' ');
    if (!transcript) {
              ...
              <div className={practiceData?.logo_url ? '' : 'w-full'}>
                ...
                <div className="mt-2">
                  <p className="text-xs sm:text-sm text-muted-foreground">
                    <span className="font-medium">Meeting:</span> {meetingData?.title || 'General Meeting'} | <span className="font-medium">Type:</span> {meetingSettings.meetingType.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase())} | <span className="font-medium">Date & Time:</span> {meetingData?.startTime ? new Date(meetingData.startTime).toLocaleDateString('en-GB') + ' at ' + new Date(meetingData.startTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'Not specified'}
                  </p>
                </div>
              </div>
            </div>
          ...
          {aiGeneratedMinutes ? (
            <>
                  ...
                  <Collapsible open={isTranscriptOpen} onOpenChange={setIsTranscriptOpen}>
                      ...
                      <CardHeader className="cursor-pointer hover:bg-accent/50 transition-colors">
                        <CardTitle className="flex items-center justify-between">
                          <span className="flex items-center gap-2">
                            ...
                            Meeting Transcript ({transcriptData.length} entries)
                          </span>
                          <ChevronDown 
                    ...
                    <CollapsibleContent>
                      <CardContent>
                            ...
                            {transcriptData.map((entry, index) => (
                              <div key={index} className="bg-background rounded-lg p-4 border border-border shadow-sm">
                                <div className="flex items-center gap-3 mb-2">
                                  <div className="bg-primary/10 text-primary px-2 py-1 rounded-md text-xs font-mono">
                                    {Math.floor(entry.timestamp_seconds / 60)}:{(entry.timestamp_seconds % 60).toString().padStart(2, '0')}
                                  </div>
                                  <div className="font-semibold text-primary">
                                    {entry.speaker_name || `Speaker ${index + 1}`}
                                  </div>
                                </div>
                                <div className="text-foreground leading-relaxed pl-2 border-l-2 border-primary/20">
                                  {entry.content}
                                </div>
                              </div>
                            ))}
                        ...
                        <div className="mt-4 flex items-center gap-2 text-xs text-muted-foreground bg-blue-50 dark:bg-blue-950/20 p-3 rounded-lg border border-blue-200 dark:border-blue-800">
                          <div className="h-2 w-2 bg-blue-500 rounded-full flex-shrink-0"></div>
                          <p>This transcript was generated automatically and may contain minor inaccuracies.</p>
                        </div>
                      </CardContent>
                    </CollapsibleContent>
                  </Collapsible>
              ...
              {audioSegments.length > 0 && (
                <Card>
                  <Collapsible open={isAudioSegmentsOpen} onOpenChange={setIsAudioSegmentsOpen}>
                    <CollapsibleTrigger asChild>
                      <CardHeader className="cursor-pointer hover:bg-accent/50 transition-colors">
                        <CardTitle className="flex items-center justify-between">
                          <span className="flex items-center gap-2">
                            <Download className="h-5 w-5" />
                            Audio Recordings ({audioSegments.length} segments)
                          </span>
                          <ChevronDown 
                            className={`h-4 w-4 transition-transform ${isAudioSegmentsOpen ? 'rotate-180' : ''}`}
                          />
                        </CardTitle>
                      </CardHeader>
                    </CollapsibleTrigger>
                      ...
                      <CardContent>
                        <div className="space-y-3">
                          <p className="text-sm text-muted-foreground mb-4">
                            Download audio recordings from this meeting. Each segment is approximately 10 minutes.
                          </p>
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  ...
                                  <p className="text-xs text-muted-foreground">
                                    ...
                                    Size: {(segment.file_size / (1024 * 1024)).toFixed(1)} MB
                                  </p>
                                  <p className="text-xs text-muted-foreground">
                      ...
                      {isTranscriptOpen && (meetingData?.transcript || transcriptData.length > 0) && (
                        <Button
                          ...
                          onClick={async (e) => {
                            e.stopPropagation();
                            try {
                              const transcriptToClean = meetingData?.transcript || transcriptData.map(t => t.content).join(' ');
                              
                              const { data, error } = await supabase.functions.invoke('clean-transcript', {
                                body: { transcript: transcriptToClean }
                              });
                              
                              if (error) throw error;
                              
                              if (data?.cleanedTranscript) {
                                // Update the meeting data with cleaned transcript
                  ...
                  {(meetingData?.transcript || transcriptData.length > 0) ? (
                    <div className="bg-accent/30 p-4 rounded-lg max-h-96 overflow-y-auto">
                      <pre className="whitespace-pre-wrap text-sm leading-relaxed font-sans">
                        {meetingData?.transcript || transcriptData.map(t => t.content).join(' ')}
                      </pre>
                    </div>
                  ) : (
