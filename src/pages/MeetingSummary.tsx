  ...
  const generateAIMeetingMinutes = async () => {
    ...
    try {
      ...
      const parseAIResponse = (content: string) => {
        ...
        const attendeesMatch = content.match(/1️⃣ Attendees\s*([\s\S]*?)(?=2️⃣|$)/i);
        if (attendeesMatch) sections.attendees = attendeesMatch[1].trim();

        // Extract agenda section
        const agendaMatch = content.match(/2️⃣ Agenda\s*([\s\S]*?)(?=3️⃣|$)/i);
        if (agendaMatch) sections.agenda = agendaMatch[1].trim();

        // Extract discussion summary as key points
        const discussionMatch = content.match(/3️⃣ Discussion Summary\s*([\s\S]*?)(?=4️⃣|$)/i);
        if (discussionMatch) sections.keyPoints = discussionMatch[1].trim();

        // Extract actions/decisions
        const actionsMatch = content.match(/4️⃣ Actions \/ Decisions\s*([\s\S]*?)(?=5️⃣|$)/i);
        if (actionsMatch) {
          const actionsText = actionsMatch[1].trim();
          // Try to separate decisions and action items
          sections.decisions = actionsText;
          sections.actionItems = actionsText;
        }

        // Extract next meeting
        const nextMeetingMatch = content.match(/5️⃣ Next Meeting\s*([\s\S]*?)$/i);
        if (nextMeetingMatch) sections.nextSteps = nextMeetingMatch[1].trim();

        return sections;
      };

      const parsedSections = parseAIResponse(meetingMinutes);
      
      setSummaryContent(prev => ({
        ...prev,
        ...parsedSections,
        additionalNotes: `Generated by AI on ${new Date().toLocaleDateString('en-GB')}\n\n--- Full AI Response ---\n${meetingMinutes}`
      }));

      toast.success("AI meeting minutes generated successfully!");
      setIsEditing(true); // Allow editing of the generated content
