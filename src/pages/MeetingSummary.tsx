...
Generated by Notewell AI Meeting Notes Service`;
  ...
  const getMeetingDate = (): string => {
    console.log('🔍 getMeetingDate - meetingSettings.date:', meetingSettings?.date);
    console.log('🔍 getMeetingDate - meetingData.startTime:', meetingData?.startTime);
    
    // Prefer MeetingSettings date if provided; fallback to meetingData.startTime; else empty
    if (meetingSettings?.date && meetingSettings.date !== '') {
      try {
    ...
    if (meetingData?.startTime && meetingData.startTime !== '') {
      try {
  ...
  const getMeetingRoundedTime = (): string => {
    console.log('🔍 getMeetingRoundedTime - meetingSettings.startTime:', meetingSettings?.startTime);
    console.log('🔍 getMeetingRoundedTime - meetingData.startTime:', meetingData?.startTime);
    
    // Prefer MeetingSettings startTime if provided; fallback to meetingData.startTime; else empty
    if (meetingSettings?.startTime && meetingSettings.startTime !== '') {
      try {
        const now = new Date();
    ...
    if (meetingData?.startTime && meetingData.startTime !== '') {
      try {
        const date = new Date(meetingData.startTime);
        if (!isNaN(date.getTime())) {
          return roundToNearestQuarterHour(date);
        }
      } catch {
        // ignore parse errors
      }
  ...
  const generateClaudeMeetingMinutes = async (customDetailLevel?: string) => {
    if (!customDetailLevel) {
      // If no style is provided, open the style selector
    ...
    const levelToUse = styleMapping[customDetailLevel] || customDetailLevel || claudeDetailLevel;
  ...
  const handleStyleSelect = (styleId: string) => {
    generateClaudeMeetingMinutes(styleId);
  };
