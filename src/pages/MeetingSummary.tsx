              ...
              <div className={practiceData?.logo_url ? '' : 'w-full'}>
                ...
                <div className="mt-2 space-y-1">
                  <p className="text-xs sm:text-sm text-muted-foreground">
                    <span className="font-medium">Meeting:</span> {meetingData.title}
                  </p>
                  <p className="text-xs sm:text-sm text-muted-foreground">
                    <span className="font-medium">Type:</span> {meetingSettings.meetingType.replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase())}
                  </p>
                  <p className="text-xs sm:text-sm text-muted-foreground">
                    <span className="font-medium">Date & Time:</span> {new Date(meetingData.startTime).toLocaleDateString()} at {new Date(meetingData.startTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                  </p>
                </div>
              </div>
            </div>
