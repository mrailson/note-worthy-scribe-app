  ...
  const handleCopyTranscript = async () => {
    try {
      ...
      if (transcriptData.length > 0) {
        ...
        transcriptText = transcriptData.map((segment, index) => {
          ...
          return `${timeStamp}${segment.speaker_name ? ` - ${segment.speaker_name}` : ''}\n${segment.content}\n`;
        }).join('\n');
...
Speakers detected: ${meetingData?.speakerCount || 0}`;
  ...
  const downloadDocx = async () => {
    try {
      ...
      // Insert canonical Date/Time header when using AI-generated minutes
      if (aiGeneratedMinutes && meetingData?.startTime) {
        const dateStr = getMeetingDate();
        const timeStr = getMeetingRoundedTime();
        documentChildren.push(new Paragraph({ children: [new TextRun({ text: `Date: ${dateStr}`, bold: true, size: 22 })], spacing: { after: 80 } }));
        documentChildren.push(new Paragraph({ children: [new TextRun({ text: `Time: ${timeStr}`, bold: true, size: 22 })], spacing: { after: 200 } }));
      }
...
Generated by Notewell AI Meeting Notes Service`;
  ...
  const getMeetingRoundedTime = (): string => {
    return meetingData?.startTime
      ? formatRoundedTimeDot(new Date(meetingData.startTime))
      : formatRoundedTimeDot(new Date());
  };
  ...
  const createTimestampedTranscript = (): string => {
    ...
    return transcriptData.map(segment => {
      // Calculate actual time by adding timestamp_seconds to meeting start time
      const segmentTime = new Date(meetingStartTime.getTime() + (segment.timestamp_seconds * 1000));
      const timeStamp = roundToNearestQuarterHour(segmentTime);
      
      const speakerName = segment.speaker_name || 'Unknown Speaker';
      return `[${timeStamp}] ${speakerName}: ${segment.content}`;
    }).join('\n\n');
};
  ...
  const generateAIMeetingMinutes = async (customDetailLevel?: string) => {
    ...
    const timestampedTranscript = createTimestampedTranscript();
    const fallbackTranscript = meetingData?.transcript || transcriptData.map(t => t.content).join(' ');
    const transcript = timestampedTranscript || fallbackTranscript;
    ...
    try {
      ...
      const response = await supabase.functions.invoke('generate-meeting-minutes', {
        body: {
          ...
          meetingStyle: meetingSettings.meetingStyle,
          attendees: meetingSettings.attendees,
          agenda: meetingSettings.agenda,
          description: meetingSettings.description,
          meetingFormat: meetingData.meetingFormat || 'teams',
          practiceId: meetingData.practiceId
        }
      });
      ...
      const cleaned = cleanAIMinutes(meetingMinutes);
      
      // Store the cleaned AI-generated document
      setAiGeneratedMinutes(cleaned);

      // Save the cleaned summary to the database
      await saveSummaryToDatabase(cleaned);

      // Only show AI generation success toast on desktop
      if (!isMobile) {
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:gap-4">
              ...
              <div className={practiceData?.logo_url ? '' : 'w-full'}>
                ...
                <p className="text-xs sm:text-sm md:text-base text-muted-foreground mt-1">
                  {practiceData?.practice_name ? `${practiceData.practice_name} - ` : ''}NHS Meeting Summary & Minutes
                </p>
                <div className="mt-2">
                  <p className="text-xs sm:text-sm text-muted-foreground">
                    <span className="font-medium">Meeting:</span> {meetingData?.title || 'General Meeting'} | <span className="font-medium">Type:</span> {meetingSettings.meetingType.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase())} | <span className="font-medium">Date & Time:</span> {meetingData?.startTime ? getMeetingDate() + ' at ' + getMeetingRoundedTime() : 'Not specified'}
                  </p>
                </div>
              </div>
            </div>
                    ...
                    <CardHeader className="cursor-pointer hover:bg-accent/50 transition-colors">
                      <CardTitle className="flex items-center justify-between">
                        <span className="flex items-center gap-2">
                          ...
                          <span className="text-sm sm:text-base">AI Generated Meeting Minutes</span>
                        </span>
                        <div className="flex items-center gap-2">
                                ...
                                <div className="w-28">
                                  <Slider
                                    ...
                                    onValueChange={(v) => {
                                      ...
                                      generateAIMeetingMinutes(newLevel);
                                    }}
                                  />
                                </div>
                          ...
                          <ChevronDown 
                            className={`h-4 w-4 transition-transform ${isAIMinutesOpen ? 'rotate-180' : ''}`}
                          />
                        </div>
                      </CardTitle>
                    </CardHeader>
                    ...
                    <CardContent>
                      {aiGeneratedMinutes ? (
                        ...
                        <div className="text-center py-8 sm:py-12 text-muted-foreground">
                          ...
                          <h3 className="text-lg sm:text-xl font-semibold mb-2">No AI Minutes Generated Yet</h3>
                          <p className="text-sm sm:text-base mb-4 max-w-md mx-auto">
                            Use the settings above to configure your meeting details and then generate AI-powered meeting minutes.
                          </p>
                          <Button
                            onClick={() => generateAIMeetingMinutes()}
                            disabled={isGeneratingMinutes}
                        ...
                        <Button
                          ...
                          onClick={async (e) => {
                            ...
                            try {
                              ...
                              const transcriptToClean = createTimestampedTranscript() || meetingData?.transcript || transcriptData.map(t => t.content).join(' ');
                              ...
                              const { data, error } = await supabase.functions.invoke('clean-transcript', {
                                body: { 
                                  rawTranscript: transcriptToClean, 
                                  meetingTitle: meetingData?.title || 'Meeting'
                                }
                              });
                              
                              console.log('📤 Clean transcript response:', { data, error });
                              ...
                              if (!data?.cleanedTranscript) {
                                throw new Error('No cleaned transcript returned from API');
                              }
                              
                              console.log('✅ Received cleaned transcript, length:', data.cleanedTranscript.length);
                              ...
                              if (meetingData?.id) {
                                ...
                                const { error: insertError } = await supabase
                                  ...
                                  .insert({
                                    meeting_id: meetingData.id,
                                    content: data.cleanedTranscript,
                                ...
                                const improvement = ((transcriptToClean.length - data.cleanedTranscript.length) / transcriptToClean.length * 100).toFixed(1);
                                ...
                                setMeetingData(prev => prev ? { ...prev, transcript: data.cleanedTranscript } : null);
