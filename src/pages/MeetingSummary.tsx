              ...
              <div className={practiceData?.logo_url ? '' : 'w-full'}>
                ...
                <div className="mt-2">
                  <p className="text-xs sm:text-sm text-muted-foreground">
                    <span className="font-medium">Meeting:</span> {meetingData.title} | <span className="font-medium">Type:</span> {meetingSettings.meetingType.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase())} | <span className="font-medium">Date & Time:</span> {meetingData.startTime ? new Date(meetingData.startTime).toLocaleDateString() + ' at ' + new Date(meetingData.startTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'Not specified'}
                  </p>
                </div>
              </div>
            </div>
