...
import { DropdownMenu, DropdownMenuContent, DropdownMenuGroup, DropdownMenuItem, DropdownMenuTrigger, DropdownMenuSub, DropdownMenuSubContent, DropdownMenuSubTrigger } from "@/components/ui/dropdown-menu";
import { MeetingSummaryTabs } from "@/components/MeetingSummaryTabs";
import { Header } from "@/components/Header";
import { NotewellAIAnimation } from "@/components/NotewellAIAnimation";

import MeetingNotesWordExport from "@/components/MeetingNotesWordExport";
...
export default function MeetingSummary() {
  ...
  // Tab state for switching between Meeting Notes and Transcript
        ...
        (async () => {
          try {
            const { data: latest, error } = await supabase
              ...
              .maybeSingle();
            if (!error && latest?.id) {
              console.log('✅ Loaded latest meeting for user:', latest);
              
              // Set comprehensive meeting data
              setMeetingData({
                id: latest.id,
                title: latest.title || 'Meeting',
                startTime: latest.start_time,
                duration: '00:00', // Default value since not in DB
                transcript: '', // Default value since not in DB  
                wordCount: 0, // Default value since not in DB
                practiceName: '', // Default value since not in DB
                speakerCount: 0
              });

              // Update meeting settings for proper display
              const startDate = latest.start_time ? new Date(latest.start_time) : null;
              const pad = (n: number) => n.toString().padStart(2, '0');
              ...
              console.log('No meetings found for user or error:', error);
            }
          } catch (err) {
            console.error('Error loading latest meeting:', err);
          }
        })();
      } else {
  ...
  const reprocessFromBackup = async () => {
    ...
    try {
      ...
      // Save the raw transcript without cleaning
      setReprocessStatus('Saving transcript...');
      console.log('💾 Saving raw transcript, length:', raw.length);
      if (meetingData?.id) {
      ...
      setMeetingData(prev => prev ? { ...prev, transcript: raw } : null);
      ...
      toast.success('Full transcript rebuilt from audio backup');
...
Speakers detected: ${meetingData?.speakerCount || 0}`;
    ...
    try {
      const { data, error } = await supabase.functions.invoke('enhance-meeting-minutes', {
        body: {
          ...
          specificRequest: customInstruction,
          context: ''
        }
      });
      ...
      await saveSummaryToDatabase(data.enhancedContent);
      toast.success("Custom instructions applied successfully!");
      
      setCustomInstruction("");
      setShowCustomInstruction(false);
    } catch (error) {
      console.error('Error applying custom instructions:', error);
      toast.error(error instanceof Error ? error.message : 'Failed to apply custom instructions');
    }
  };
  ...
  const downloadPDF = async () => {
    try {
      // Save the current summary to database before downloading
      if (claudeNotes) {
        await saveSummaryToDatabase(claudeNotes);
      }
      
      const content = claudeNotes || 'No content available';
      const pdf = new jsPDF();
      const pageWidth = pdf.internal.pageSize.getWidth();
      const pageHeight = pdf.internal.pageSize.getHeight();
      const margin = 20;
      ...
      pdf.save(`${meetingData?.title || 'meeting'}_claude_notes.pdf`);
...
Generated by Notewell AI Meeting Notes Service`;
  ...
  const saveSummaryToDatabase = async (summaryContent: string) => {
    ...
    console.log('💾 Attempting to save summary to database for meeting:', meetingData.id);
    ...
    try {
      ...
      console.log('💾 Existing summary check:', existingSummary ? 'Found existing' : 'No existing summary');
      ...
      if (existingSummary) {
        ...
        if (error) throw error;
        console.log('✅ Summary updated successfully in database');
      } else {
        // Create new summary
        const { error } = await supabase
          .from('meeting_summaries')
  ...
  const saveEditedContent = async () => {
    console.log('🔄 Saving edited content:', editableContent.substring(0, 100) + '...');
  ...
  const loadExistingSummary = async (meetingId: string) => {
    console.log('📖 Loading existing summary for meeting:', meetingId);
    try {
      ...
      console.log('📖 Summary query result:', summaryResult);

      if (summaryResult.error) {
        console.error('❌ Error loading existing summary:', summaryResult.error);
      } else if (summaryResult.data?.summary) {
        console.log('✅ Loaded existing summary from database, length:', summaryResult.data.summary.length);
        console.log('📖 Summary preview:', summaryResult.data.summary.substring(0, 200) + '...');
        setAiGeneratedMinutes(summaryResult.data.summary);
        setClaudeNotes(summaryResult.data.summary); // Also set as Claude notes
      } else {
        console.log('📖 No existing summary found in database');
      }
      ...
      if (overviewResult.error) {
        ...
        // Parse the overview HTML to extract meeting details
        try {
          const overviewHtml = overviewResult.data.overview;
          console.log('🔍 Raw overview HTML:', overviewHtml);
          ...
          const title = titleElement ? titleElement.textContent?.replace(/Saxon Spires\.?\s*/i, '').trim() || 'Meeting' : 'Meeting';
          console.log('🔍 Extracted title:', title);
          
          // Extract date and time from the paragraph containing "Date:"
          const dateTimePattern = /Date:\s*(\d{1,2}[a-z]{2}\s+[A-Za-z]+\s+\d{4})\s+Time:\s*(\d{1,2}:\d{2})/i;
          const match = overviewHtml.match(dateTimePattern);
          console.log('🔍 Date/time pattern match:', match);
          ...
          if (match) {
            const dateStr = match[1]; // e.g., "19th August 2025"
            const timeStr = match[2]; // e.g., "19:00"
            console.log('🔍 Matched date string:', dateStr, 'time string:', timeStr);
            
            // Parse the date string
            const cleanDateStr = dateStr.replace(/(\d+)(st|nd|rd|th)/, '$1');
            const parsedDate = new Date(cleanDateStr);
            console.log('🔍 Clean date string:', cleanDateStr, 'parsed date:', parsedDate);
            
            if (!isNaN(parsedDate.getTime())) {
              const pad = (n: number) => n.toString().padStart(2, '0');
              date = `${parsedDate.getFullYear()}-${pad(parsedDate.getMonth() + 1)}-${pad(parsedDate.getDate())}`;
              
              // Create full datetime for meetingData
              const [hours, minutes] = timeStr.split(':').map(Number);
              parsedDate.setHours(hours, minutes, 0, 0);
              meetingStartTime = parsedDate.toISOString();
              startTime = timeStr;
              console.log('🔍 Final parsed values:', { date, startTime, meetingStartTime });
            }
          }
          
          console.log('🔍 About to update states with:', { title, date, startTime, meetingStartTime });
          
          // Update meetingData with parsed information
          setMeetingData(prev => {
            const newData = prev ? {
              ...prev,
              title: title,
            ...
            console.log('🔍 Updated meetingData:', newData);
            return newData;
          });
          
          // Update meetingSettings for proper display
          setMeetingSettings(prev => {
            const newSettings = {
              ...prev,
              title: title,
            ...
            console.log('🔍 Updated meetingSettings:', newSettings);
            return newSettings;
          });
          
        } catch (parseError) {
          console.error('Error parsing meeting overview:', parseError);
        }
      }
  ...
  return (
    <>
      <div className="min-h-screen bg-background">
        ...
        {/* Back Button - Truly Left Aligned */}
        ...
        {/* Tabs for switching between Meeting Notes and Transcript */}
        <MeetingSummaryTabs
          activeTab={activeTab}
          ...
          meetingNotesContent={
            aiGeneratedMinutes ? (
              ...
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p>No meeting notes generated yet</p>
              </div>
            )
          }
          transcriptContent={
            <Card>
              ...
              <CardContent>
                <div className="prose prose-sm max-w-none">
                  <pre className="whitespace-pre-wrap">{meetingData?.transcript || 'No transcript available'}</pre>
                </div>
              </CardContent>
            </Card>
          }
        ...
        {meetingData?.id && (
          ...
          />
                              ...
                              {(() => {
                                ...
                                if (meetingData?.startTime) {
                                  try {
                                    ...
                                    console.log('  Error parsing meetingData.startTime:', e);
                                  }
                                } else if (meetingSettings?.date) {
                                  try {
                                    const [y, m, d] = meetingSettings.date.split('-').map(Number);
                                    ...
                                    console.log('  Error parsing meetingSettings.date:', e);
                                  }
                                }
                                ...
                                if (meetingData?.startTime) {
                                  try {
                                    ...
                                    console.log('  Error parsing meetingData.startTime for time:', e);
                                  }
                                } else if (meetingSettings?.startTime) {
                                  time = meetingSettings.startTime;
                                }
                                
                                const wordCount = meetingData?.wordCount ? `${(meetingData.wordCount / 1000).toFixed(1)}K` : '0';
                                
                                console.log('  Final values:', { title, date, time, wordCount });
                                return `${title} • ${date} • ${time} • ${wordCount} Words - Transcript`;
                              })()}
                            </div>
                          </div>
                        </span>
                          ...
                          {isClaudeGenerating && (
                           ...
                           {claudeNotes && (
                            <div className="flex items-center gap-1">
                              ...
                              <MeetingNotesWordExport 
                                meetingData={{
                                  title: meetingData?.title || 'Meeting Notes',
                                  date: getMeetingDate(),
                                  duration: meetingData?.duration || '00:00',
                                  attendees: summaryContent.attendees || meetingSettings.attendees || 'Meeting attendees',
                              ...
                              />
                               <ClaudeEnhancementModal
                                 ...
                                 onEnhancedContent={(enhancedContent) => {
                                   ...
                                   toast.success("Claude notes enhanced and saved!");
                                 }}
                               >
                                 <Button
                                   variant="outline"
                               ...
                               <Button
                                 onClick={async (e) => {
                                   e.stopPropagation();
                                   console.log('🔄 Claude Edit/Save button clicked, isClaudeEditing:', isClaudeEditing);
                                   if (isClaudeEditing) {
                                     console.log('💾 Saving Claude notes, length:', claudeNotes.length);
                                     console.log('💾 Claude notes content preview:', claudeNotes.substring(0, 100));
                                     // Save the Claude notes to database
                                     await saveSummaryToDatabase(claudeNotes);
                                     toast.success("Claude notes saved successfully!");
                                   }
                          ...
                          <div className="border rounded-lg overflow-hidden bg-white">
                            {isClaudeEditing ? (
                              <Textarea
                                ...
                                className="min-h-[400px] sm:min-h-[600px] font-mono text-sm leading-relaxed resize-none border-0 focus:ring-0 focus:outline-none bg-white"
                                style={{ 
                                  whiteSpace: 'pre-wrap',
              ...
              {/* Remove the duplicate Claude AI section that was moved to top */}
                ...
                /* Initial Summary View - Only Meeting Notes Available */
                <div className="text-center py-8 sm:py-12 text-muted-foreground">
                  <Bot className="h-12 w-12 sm:h-16 sm:w-16 mx-auto mb-4 opacity-50" />
                  <h3 className="text-lg sm:text-xl font-semibold mb-2">No Meeting Notes Generated Yet</h3>
                  <p className="text-sm sm:text-base mb-4 max-w-md mx-auto">
                    Generate enhanced meeting notes using Claude AI.
                  </p>
                </div>
              )}
            </div>
    ...
    {/* Full Screen Claude Notes Modal */}
        ...
        <DialogHeader className="p-6 pb-4 border-b flex-shrink-0">
          <div className="flex items-center justify-between">
            <DialogTitle className="flex items-center gap-2">
              ...
              Claude AI Meeting Minutes - Full Screen View
            </DialogTitle>
            <Button
              onClick={() => setIsClaudeFullScreen(false)}
              variant="ghost"
              size="sm"
          ...
          <div className="flex items-center gap-2 mt-4 pt-4 border-t">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="start" className="w-56 bg-white border shadow-md z-50">
                <DropdownMenuGroup>
                  {/* Edit Submenu */}
                  <DropdownMenuSub>
                    ...
                    <DropdownMenuSubContent className="bg-white border shadow-md z-50">
                      <DropdownMenuItem onClick={() => {
                        // Edit the content based on active tab
                        if (activeTab === "meeting-notes") {
                          setIsClaudeEditing(true);
                        } else {
                          // Handle transcript editing
                          toast.info("Transcript editing feature coming soon!");
                        }
                      }}>
                        <FileText className="h-4 w-4 mr-2" />
                        Edit {activeTab === "meeting-notes" ? "Meeting Notes" : "Transcript"}
                      </DropdownMenuItem>
                      ...
                      <DropdownMenuItem onClick={() => {
                        // Handle Update Names and Terms based on active tab
                        if (activeTab === "meeting-notes") {
                          toast.info("Update Names and Terms for Meeting Notes coming soon!");
                        } else {
                          toast.info("Update Names and Terms for Transcript coming soon!");
                        }
                      }}>
                    ...
                    <DropdownMenuSubContent className="bg-white border shadow-md z-50">
                      <DropdownMenuItem onClick={async (e) => {
                        ...
                        try {
                          ...
                          const doc = new Document({
                            sections: [{
                              children: [
                                ...
                                ...stripHtmlAndFormat(claudeNotes)
                              ]
                            }]
                          });
                          ...
                          link.href = url;
                          link.download = `Meeting_Notes_${getMeetingDate()}.docx`;
                      ...
                      <DropdownMenuItem onClick={() => {
                        ...
                        const file = new Blob([claudeNotes || ''], {type: 'text/plain'});
            ...
            <div className="p-4 border-b bg-gray-50">
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  />
                  <SpeechToText 
                    onTranscription={(text) => setCustomInstruction(prev => prev ? `${prev} ${text}` : text)}
                    size="sm"
                    className="h-10"
                  />
                </div>
