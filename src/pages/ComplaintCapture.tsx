...
export default function ComplaintCapture() {
  ...
  const validateSession = async () => {
    try {
      ...
      if (!data.is_active || expiresAt < now) {
        setValidationError('This session has expired. Please scan a new QR code.');
        setIsValid(false);
        setIsValidating(false);
        return;
      }
  ...
  const startCamera = useCallback(async (deviceId?: string) => {
    ...
    try {
      ...
      setIsCameraLoading(false);
      showToast.error('Failed to access camera. Please use file upload instead.');
    }
  }, [enumerateCameras]);
        ...
        <Card className="w-full max-w-md">
          ...
          <CardContent className="text-center">
            ...
            <p className="text-sm text-muted-foreground mt-2">
              Please scan a new QR code from the Complaints system.
            </p>
          </CardContent>
        </Card>
      </div>
  ...
  return (
    <div className="min-h-[100dvh] bg-background p-4 pb-[calc(env(safe-area-inset-bottom)+4rem)]">
      <div className="max-w-lg mx-auto space-y-4 pb-8">
        <Card>
          <CardHeader className="pb-2">
            ...
            <p className="text-sm text-muted-foreground">
              Take photos of patient letters or handwritten complaints
            </p>
          </CardHeader>
              ...
              >
                ...
                {isCapturing && glareWarning && (
                  <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-yellow-500 text-black px-4 py-2 rounded-lg flex items-center gap-2 z-10">
                    ...
                    <span className="text-sm font-medium">Glare detected - try adjusting angle</span>
                  </div>
                )}
            ...
            <CardContent className="space-y-4">
              ...
              {uploadedCount > 0 && pendingCount === 0 && (
                <div className="text-center text-green-600 font-medium">
                  ...
                  All photos uploaded and synced to desktop!
                </div>
              )}
            </CardContent>
          </Card>
        ...
        {pendingCount > 0 && (
          <div className="sm:hidden fixed bottom-0 left-0 right-0 px-4 pt-3 pb-[calc(env(safe-area-inset-bottom)+1rem)] bg-background/95 backdrop-blur border-t border-border z-50">
