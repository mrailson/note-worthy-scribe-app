...
const ComplaintDetails = () => {
  ...
  const triggerPostOutcomeSentTasks = (currentComplaint: Complaint) => {
    ...
    // 3. Training PowerPoint (6 slides via Gamma)
    supabase.functions.invoke('generate-complaint-ai-report', {
      body: { complaintId: currentComplaint.id }
    }).then(async ({ data: reportData, error: reportError }) => {
      if (reportError || !reportData) {
        console.error('[Background] AI report fetch for PowerPoint failed:', reportError);
        return;
      }
      ...
      try {
        ...
        // Look up practice name
        let practiceName = '';
        if (currentComplaint.practice_id) {
          const { data: practiceDetails } = await supabase
            .from('practice_details')
            .select('practice_name')
            .eq('id', currentComplaint.practice_id)
            .maybeSingle();
        ...
        const supportingContent = [
          '# Learning Together: Complaint Review',
          '## A Staff Training Presentation',
          practiceName ? `### ${practiceName}` : '',
          '',
          `Reference: ${currentComplaint.reference_number} | Category: ${currentComplaint.category}`,
          `Date Received: ${formattedDate}`,
          '',
          '### What Happened',
          reportData.complaintOverview?.substring(0, 500) || '',
          outcomeTypeValue ? `Outcome: ${outcomeTypeValue.split('_').map((w: string) => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')}` : '',
          '',
          '### Key Learnings',
          ...(reportData.keyLearnings || []).slice(0, 5).map((l: any, i: number) => `${i + 1}. ${l.learning} (${l.category})`),
          ...
          '',
          '### How We Are Improving',
          ...(reportData.improvementSuggestions || []).slice(0, 4).map((s: any, i: number) => `${i + 1}. ${s.suggestion} [${s.priority} priority]`),
          '',
          '### Thank You & Discussion',
          practiceName || '',
          'Questions and reflections from the team',
          'Powered by NoteWell AI',
        ].join('\n');

        const practiceNameInstruction = practiceName
          ? `BRANDING: Include the practice name "${practiceName}" on the title and final slides.`
          : 'BRANDING: No practice name available — keep branding generic.';

        const customInstructions = [
          'PURPOSE: This presentation is for shared learning and system improvement, not to attribute fault.',
          'TONE: Warm, supportive PLT training tone. Frame issues as system learning, not individual error.',
          'PRIVACY: Fully anonymised. No patient or staff names.',
          'JUST CULTURE: Use collective terms. Do not single out specific staff groups.',
          'LIABILITY-SAFE: No confirmed financial loss or harm language.',
          practiceNameInstruction,
          'DESIGN: NHS-inspired palette with photorealistic GP/primary care imagery.',
          'TYPOGRAPHY: Use only standard Office fonts (Calibri, Arial). No non-standard fonts.',
          'Speaker notes in hidden notes pane only.',
          'STRUCTURE: Exactly 6 slides.',
        ].join(' ');
        ...
        const { data: startResponse, error: startError } = await supabase.functions.invoke(
          ...
          {
            body: {
              topic: `Learning Together: Complaint Review — ${currentComplaint.reference_number} (${currentComplaint.category})`,
              presentationType: 'NHS Staff Training Session',
              slideCount: 6,
              supportingContent,
              customInstructions,
              audience: 'NHS GP practice staff during Protected Learning Time sessions',
              fontStyle: 'modern',
        ...
        if (startError || !startResponse?.success || !startResponse?.generationId) {
          console.error('[Background] PowerPoint generation failed to start:', startError || startResponse?.error);
          return;
        }

        const generationId = startResponse.generationId;
        console.log('[Background] PowerPoint generation started, ID:', generationId);

        // Poll for completion
        const maxPollTime = 6 * 30000; // 30s per slide
        ...
        while (Date.now() - startTime < maxPollTime) {
          ...
          if (pollError) {
            if (pollError.message?.includes('429') || pollError.message?.includes('ThrottlerException')) {
              pollInterval = Math.min(pollInterval * 2, 120000);
              continue;
            }
            console.error('[Background] PowerPoint poll error:', pollError);
            continue;
          }

          if (pollResponse?.status === 'completed' && pollResponse?.downloadUrl) {
            console.log('[Background] PowerPoint generation completed');

            // Persist metadata (thumbnail + URLs) — no auto-download
            ...
            const pptData = {
              powerpoint_download_url: pollResponse.downloadUrl,
              powerpoint_gamma_url: pollResponse.gammaUrl || null,
              powerpoint_slide_count: 6,
            };

            if (existing) {
              await supabase.from('complaint_audio_overviews').update(pptData as any).eq('complaint_id', currentComplaint.id);
            } else {
              const { data: userData } = await supabase.auth.getUser();
              await supabase.from('complaint_audio_overviews').insert({
            ...
            console.log('[Background] PowerPoint persisted successfully');
            fetchComplaintDetails();
            break;
          }

          if (pollResponse?.status === 'failed') {
            console.error('[Background] PowerPoint generation failed:', pollResponse?.error);
            break;
          }
        ...
        console.error('[Background] PowerPoint generation error:', pptErr);
      }
    });
  };
    ...
    try {
      const { data, error } = await supabase.functions.invoke('regenerate-outcome-letter', {
        body: {
          ...
          useFormalLabels: outcomeQuestionnaireData?.use_formal_outcome_labels ?? true
        }
      });
