            ...
            <CardContent>
              ...
              <div className="mt-6 p-4 bg-muted/30 rounded-lg">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div>
                    <span className="font-medium">Received:</span> {complaint.created_at ? format(new Date(complaint.created_at), 'dd/MM/yyyy') : 'Not recorded'}
                  </div>
                  {complaint.submitted_at && (
                    <div>
                      <span className="font-medium">Submitted:</span> {format(new Date(complaint.submitted_at), 'dd/MM/yyyy')}
                    </div>
                  )}
                  {acknowledgementDate && (
                    <div>
                      <span className="font-medium">Acknowledged:</span> {format(new Date(acknowledgementDate), 'dd/MM/yyyy')}
                    </div>
                  )}
                  {existingOutcome?.decided_at && (
                    <div>
                      <span className="font-medium">Outcome:</span> {format(new Date(existingOutcome.decided_at), 'dd/MM/yyyy')}
                    </div>
                  )}
                  {complaint.closed_at && (
                    <div>
                      <span className="font-medium">Closed:</span> {format(new Date(complaint.closed_at), 'dd/MM/yyyy')}
                    </div>
                  )}
                </div>
              </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between p-3 bg-background rounded border">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-medium">Acknowledgement Letter</div>
                        <div className="text-sm text-muted-foreground">
                          {acknowledgementLetter ? (
                            acknowledgementDate ? `Sent ${format(new Date(acknowledgementDate), 'dd/MM/yyyy')}` : 'Generated'
                          ) : 'Not generated'}
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="flex items-center justify-between p-3 bg-background rounded border">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-medium">Outcome Letter</div>
                        <div className="text-sm text-muted-foreground">
                          {outcomeLetter ? (
                            existingOutcome?.decided_at ? `Sent ${format(new Date(existingOutcome.decided_at), 'dd/MM/yyyy')}` : 'Generated'
                          ) : 'Not generated'}
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      {outcomeLetter ? (
                        ...
                        <div className="text-sm text-muted-foreground">
                          Complete investigation decision to generate
                        </div>
                      )}
                    </div>
                  </div>
