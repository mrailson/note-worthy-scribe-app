...
export function generateDOCXContent(
  ...
  const htmlContent = `
    ...
    <head>
      <meta charset="UTF-8">
      <title>Notewell AI Translation Session Report</title>
      <style>
        body { 
...
export async function downloadDOCX(
  ...
  try {
    ...
    const children = [
      ...
      new Paragraph({
        children: [
          new TextRun({
            text: isPatientCopy 
              ? (translatedContent?.title || "Notewell AI Translation Service - Patient Copy")
              : "Notewell AI Translation Service Report",
            bold: true,
            size: 32,
            color: "005EB8"
          })
    ...
    if (isPatientCopy) {
      children.push(
        new Paragraph({
          children: [new TextRun({ text: translatedContent?.patientInfo || "Patient Information", bold: true, size: 28, color: "005EB8" })],
          heading: HeadingLevel.HEADING_2
        }),
        new Paragraph({
          children: [new TextRun(translatedContent?.patientInfoText || "This document contains a record of all translations made during your medical consultation. This is provided for your personal records and reference.")]
        }),
        new Paragraph({ text: "" })
      );
    }
    ...
    const doc = new Document({
      sections: [{
        ...
        children: [
          ...
          new Paragraph({
            children: [new TextRun({
              text: isPatientCopy ? (translatedContent?.disclaimerText || "IMPORTANT DISCLAIMER: This is an automated translation service for communication assistance only. All medical decisions should be based on professional clinical judgement.") : "IMPORTANT DISCLAIMER: This is an automated translation service for communication assistance only. All medical decisions should be based on professional clinical judgement.",
              bold: true,
              size: 20
            })],
            alignment: AlignmentType.CENTER
          ...
          new Paragraph({
            children: [new TextRun({
              text: `${isPatientCopy ? (translatedContent?.reportGeneratedBy || "Report generated by Notewell AI Translation Tool") : "Report generated by Notewell AI Translation Tool"} - ${new Date().toLocaleString('en-GB')}`,
              size: 18,
              italics: true
            })],
            alignment: AlignmentType.CENTER
