// HTTP-based chunk transcription for Amazon Transcribe
...
export class AmazonTranscribeChunkTranscriber {
  ...
  async startTranscription() {
    console.log('🚀 Starting Amazon Transcribe chunk transcription...');
    ...
    try {
      ...
      console.log('✅ Amazon Transcribe chunk transcription started');
      
    } catch (error) {
      console.error('❌ Failed to start Amazon Transcribe chunk transcription:', error);
  ...
  private async processAudioChunk() {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('amazon-transcribe-chunk', {
        body: { 
          audio: base64Audio,
          mimeType: 'audio/webm;codecs=opus',
          chunkIndex
      ...
      if (error) {
        console.error('❌ Amazon Transcribe transcription error:', error);
  ...
  stopTranscription() {
    console.log('🛑 Stopping Amazon Transcribe chunk transcription...');
    this.isActive = false;
  ...
  async clearSummary() {
    console.log('Amazon Transcribe chunk transcriber does not maintain summaries');
  }
}