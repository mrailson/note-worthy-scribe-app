...
export const useAI4GPService = () => {
  ...
  const [imageGenerationModel, setImageGenerationModel] = useState<'google/gemini-2.5-flash-image' | 'google/gemini-3-pro-image-preview' | 'openai/gpt-image-1'>('google/gemini-2.5-flash-image');
  ...
  const handleBrandingConfirm = useCallback(async (
    ...
    try {
      ...
      const imageMessage: Message = {
        ...
        isStreaming: false,
        responseTime,
        model: imageGenerationModel === 'google/gemini-2.5-flash-image' ? 'Nano Banana' : 
