...
// Helper function to deep merge defaults with existing data
const mergeWithDefaults = (dbData: any, defaults: any) => {
...
const getDefaultValueProposition = () => ({
  ...
  b4_evidenceBase: 'User feedback from beta testing. Time and motion studies showing reduced documentation time. Comparative analysis with manual processes',
  b5_nhsAlignment: 'Supports NHS Long Term Plan objectives for digital transformation, supports GP contract requirements for complaints handling, improves administrative efficiency allowing more time for patient care, supports quality improvement through structured data capture',
});

const getDefaultClinicalSafety = () => ({
  c1_1_csoName: '',
...
const getDefaultDataProtection = () => ({
...
const getDefaultTechnicalSecurity = () => ({
...
const getDefaultInteroperability = () => ({
...
const getDefaultUsabilityAccessibility = () => ({
...
export const useDTACAssessment = () => {
  ...
  const loadAssessment = async () => {
    try {
      ...
      if (data) {
        // Load existing assessment and merge with defaults
        ...
        const mergedAssessment = {
          ...
          companyInfo: mergeWithDefaults(data.company_info || {}, getDefaultCompanyInfo()),
          valueProposition: mergeWithDefaults(data.value_proposition || {}, getDefaultValueProposition()),
          clinicalSafety: mergeWithDefaults(data.clinical_safety || {}, getDefaultClinicalSafety()),
          dataProtection: mergeWithDefaults(data.data_protection || {}, getDefaultDataProtection()),
          technicalSecurity: mergeWithDefaults(data.technical_security || {}, getDefaultTechnicalSecurity()),
          interoperability: mergeWithDefaults(data.interoperability || {}, getDefaultInteroperability()),
          usabilityAccessibility: mergeWithDefaults(data.usability_accessibility || {}, getDefaultUsabilityAccessibility()),
        };
        
        setAssessment(mergedAssessment);
        
        // Save the merged data back to the database to persist defaults
        const { error: updateError } = await supabase
          .from('dtac_assessments')
          .update({
            company_info: mergedAssessment.companyInfo,
            value_proposition: mergedAssessment.valueProposition,
            clinical_safety: mergedAssessment.clinicalSafety,
            data_protection: mergedAssessment.dataProtection,
            technical_security: mergedAssessment.technicalSecurity,
            interoperability: mergedAssessment.interoperability,
            usability_accessibility: mergedAssessment.usabilityAccessibility,
            updated_at: new Date().toISOString(),
          })
          .eq('id', data.id);

        if (updateError) {
          console.error('Error updating assessment with defaults:', updateError);
        }
      } else {
        ...
        setAssessment({
          ...
          valueProposition: getDefaultValueProposition(),
          clinicalSafety: getDefaultClinicalSafety(),
          dataProtection: getDefaultDataProtection(),
          technicalSecurity: getDefaultTechnicalSecurity(),
          interoperability: getDefaultInteroperability(),
          usabilityAccessibility: getDefaultUsabilityAccessibility(),
        });
      }
    } catch (error) {
