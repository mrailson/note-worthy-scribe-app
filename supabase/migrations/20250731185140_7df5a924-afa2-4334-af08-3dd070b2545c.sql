...
CREATE TABLE public.cqc_domains (
  ...
  name TEXT NOT NULL,
  description TEXT,
  weight DECIMAL(3,2) DEFAULT 20.00,
...
CREATE TABLE public.cqc_policies (
  ...
  cqc_domain TEXT REFERENCES public.cqc_domains(name),
...
CREATE TABLE public.cqc_alerts (
  ...
  due_date DATE,
  related_policy_id UUID REFERENCES public.cqc_policies(id),
  related_evidence_id UUID REFERENCES public.cqc_evidence(id),
  created_by UUID REFERENCES auth.users(id),
