-- Fix get_practice_users():
-- 1) Avoid casting '' to practice_role enum (22P02)
-- 2) user_roles.ai4gp_access does not exist; ai4gp_access lives on profiles
...
BEGIN
  ...
  ORDER BY p.id, COALESCE(ur.assigned_at, ur.created_at) DESC;
END;
$function$;
