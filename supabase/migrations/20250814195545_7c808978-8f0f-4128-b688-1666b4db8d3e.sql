-- Fix Critical Security Issues - Working with actual table structures

-- 1. Fix contractor data security (using existing structure)
-- Drop the insecure "public view" policies for contractors
...
-- Keep the secure view policy that already exists and add secure management policies
...
-- Fix contractor_resumes policies (replace public access)
...
-- Fix contractor_experience policies (replace public access)
...
-- Fix contractor_recommendations policies (replace public access)
...
-- Fix contractor_notes policies (replace public access)
...
-- 2. Fix staff_members security (using existing structure)
...
-- Secure policies based on role and practice access only
CREATE POLICY "Practice managers can view staff members" 
...
-- 3. Fix scheduling data security
...
CREATE POLICY "Practice managers can view staff assignments" 
...
-- 4. Fix security settings visibility (replace public access)
...
-- 5. Fix staff hours summary visibility (replace public access)
...
CREATE POLICY "Practice managers can view hours summary" 
