-- Fix security warnings by updating the function search paths
-- The functions were already created with proper search path, but we need to ensure they are properly set

-- Drop and recreate the functions with explicit search path if needed
DROP FUNCTION IF EXISTS public.get_practice_manager_assignable_roles();
DROP FUNCTION IF EXISTS public.check_user_practice_assignment(text, uuid);
DROP FUNCTION IF EXISTS public.get_practice_users(uuid);

-- Recreate with explicit security definer and search path
