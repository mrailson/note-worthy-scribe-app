-- Fix Critical Security Issues - Updated based on actual table structures

-- 1. Fix contractor data security
-- Enable RLS on contractors table (if not already enabled)
ALTER TABLE public.contractors ENABLE ROW LEVEL SECURITY;

-- Drop existing policies and create new secure ones for contractors
...
-- 2. Fix staff_members table (create if not exists and secure)
...
-- 3. Fix staff_assignments table (create if not exists and secure)
...
DROP POLICY IF EXISTS "Practice managers can view assignments for their practices" ON public.staff_assignments;
DROP POLICY IF EXISTS "Practice managers can manage assignments for their practices" ON public.staff_assignments;
...
-- Fix shift_templates RLS policies
ALTER TABLE public.shift_templates ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Practice managers can view shift templates for their practices" ON public.shift_templates;
DROP POLICY IF EXISTS "Practice managers can manage shift templates for their practices" ON public.shift_templates;
...
-- Fix replacement_shifts RLS policies
ALTER TABLE public.replacement_shifts ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Practice managers can view replacements for their practices" ON public.replacement_shifts;
DROP POLICY IF EXISTS "Practice managers can manage replacements for their practices" ON public.replacement_shifts;
...
DROP POLICY IF EXISTS "System admins can view security settings" ON public.security_settings;
...
DROP POLICY IF EXISTS "Practice managers can view hours summary for their practices" ON public.staff_hours_summary;
...
-- 6. Fix function search path issues for existing functions
