-- Fix get_meeting_full_transcript to check consolidated transcript first
...
BEGIN
  ...
  -- 1) Check consolidated transcript in meetings table FIRST (highest priority)
  SELECT
  ...
  -- 2) Latest session in meeting_transcription_chunks for this meeting
  ...
  -- 3) Concatenated meeting_transcripts (for the meeting owner)
  SELECT
  ...
  -- 4) Legacy transcription_chunks (for the meeting owner)
  SELECT
