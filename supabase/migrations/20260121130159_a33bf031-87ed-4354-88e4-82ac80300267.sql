-- Drop and recreate the function with Scribe and Meeting columns
...
RETURNS TABLE (
  ...
  out_scribe_count bigint,
  out_meeting_count bigint,
...
BEGIN
  ...
  scribe_stats AS (
    SELECT 
      s.user_id as uid,
      COUNT(*)::bigint as scribe_count,
      MAX(s.created_at) as last_act
    FROM gp_consultations s
    GROUP BY s.user_id
  ),
  meeting_stats AS (
    SELECT 
  ...
  all_users AS (
    ...
    UNION
    SELECT uid FROM scribe_stats
    UNION
    SELECT uid FROM meeting_stats
  )
  SELECT 
    ...
    COALESCE(sc.scribe_count, 0)::bigint as out_scribe_count,
    COALESCE(mt.meeting_count, 0)::bigint as out_meeting_count,
    ...
    GREATEST(a4.last_act, gs.last_act, sc.last_act, mt.last_act) as out_last_active
  FROM all_users au
  LEFT JOIN ai4gp_stats a4 ON au.uid = a4.uid
  LEFT JOIN genie_stats gs ON au.uid = gs.uid
  LEFT JOIN scribe_stats sc ON au.uid = sc.uid
  LEFT JOIN meeting_stats mt ON au.uid = mt.uid
