DROP FUNCTION IF EXISTS public.get_translation_usage_report();

CREATE FUNCTION public.get_translation_usage_report()
RETURNS TABLE(
  user_id uuid,
  email text,
  full_name text,
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE((SELECT COUNT(*) FROM reception_translation_messages m WHERE m.session_id IN (SELECT s2.id FROM reception_translation_sessions s2 WHERE s2.user_id = s.user_id)), 0)::bigint as total_messages,
    ...
    ROUND(
      COALESCE(
        (SELECT COUNT(*) FROM reception_translation_messages m WHERE m.session_id IN (SELECT s2.id FROM reception_translation_sessions s2 WHERE s2.user_id = s.user_id))::numeric 
        / NULLIF(COUNT(DISTINCT s.id), 0), 
        0
      ), 1
    ) as avg_messages_per_session,
    COUNT(DISTINCT s.id) FILTER (WHERE s.is_training = true)::bigint as training_sessions,
    COUNT(DISTINCT s.id) FILTER (WHERE s.is_training = false)::bigint as live_sessions
