-- Add word count breakdown by time period to meeting usage report
...
BEGIN
  ...
  meeting_stats AS (
    SELECT 
      ...
      SUM(COALESCE(NULLIF(m.transcript_cleaned_word_count, 0), m.word_count, 0)) FILTER (WHERE m.created_at >= NOW() - INTERVAL '24 hours') as words_24h,
