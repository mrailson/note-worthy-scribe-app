...
CREATE FUNCTION public.get_meeting_usage_report()
RETURNS TABLE(
  user_id uuid,
  email text,
  full_name text,
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(AVG(CASE WHEN m.word_count >= 100 THEN m.duration_mins END)::bigint, 0) as avg_duration_mins,
    COALESCE(SUM(CASE WHEN m.word_count >= 100 THEN m.duration_mins END)::bigint, 0) as total_duration_mins,
    ...
    COALESCE(SUM(CASE WHEN m.created_at >= NOW() - INTERVAL '24 hours' AND m.word_count >= 100 THEN m.duration_mins END)::bigint, 0) as duration_24h,
    COALESCE(SUM(CASE WHEN m.created_at >= NOW() - INTERVAL '7 days' AND m.word_count >= 100 THEN m.duration_mins END)::bigint, 0) as duration_7d,
    COALESCE(SUM(CASE WHEN m.created_at >= NOW() - INTERVAL '30 days' AND m.word_count >= 100 THEN m.duration_mins END)::bigint, 0) as duration_30d,
