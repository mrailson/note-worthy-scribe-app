...
-- Recreate the function to use "today" instead of "last 24 hours"
...
BEGIN
  ...
  meeting_stats AS (
    SELECT 
      m.user_id,
      -- Changed: Use CURRENT_DATE for "today" instead of 24 hours rolling window
      ...
      -- Changed: Use CURRENT_DATE for duration and words as well
  ...
  deleted_stats AS (
    ...
    FROM public.deleted_meetings dm
    GROUP BY dm.user_id
  )
  SELECT 
    u.user_id,
...
-- Create a new function to get today's meetings with details for hover tooltips
