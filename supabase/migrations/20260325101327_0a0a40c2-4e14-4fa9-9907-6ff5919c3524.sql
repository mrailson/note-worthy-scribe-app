...
CREATE OR REPLACE FUNCTION public.get_practice_users(p_practice_id uuid)
 RETURNS TABLE(user_id uuid, email text, full_name text, role text, practice_role text, meeting_notes_access boolean, gp_scribe_access boolean, complaints_manager_access boolean, ai4gp_access boolean, enhanced_access boolean, cqc_compliance_access boolean, shared_drive_access boolean, mic_test_service_access boolean, api_testing_service_access boolean, translation_service_access boolean, fridge_monitoring_access boolean, cso_governance_access boolean, lg_capture_access boolean, bp_service_access boolean, survey_manager_access boolean, document_signoff_access boolean, agewell_access boolean, last_login timestamp with time zone, assigned_at timestamp with time zone)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  SELECT DISTINCT ON (p.user_id)
    ...
    COALESCE(ur.document_signoff_access, false) as document_signoff_access,
    COALESCE(ur.agewell_access, false) as agewell_access,
  ...
  ORDER BY p.user_id, COALESCE(ur.assigned_at, ur.created_at) DESC;
END;
$function$;