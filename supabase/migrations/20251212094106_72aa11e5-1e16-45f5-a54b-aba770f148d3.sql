-- Update get_meeting_full_transcript to prioritize live_transcript_text (contains user-added context like agenda, attendees)
-- This ensures that when users add context via the TranscriptContextDialog, it's included in notes regeneration
...
BEGIN
  ...
  -- 1) Check live_transcript_text FIRST (contains user-added context like agenda, attendees)
  SELECT
  ...
  -- 2) Check consolidated whisper_transcript_text in meetings table
  SELECT
