...
Start at 100. Deduct points based on identified issues (the Notewell bonus is applied separately via the policy_source flag — do NOT try to detect it from document content):

DEDUCTIONS (use the severity from each gap object):
...
You must respond in valid JSON format with the following structure:
{
  ...
  "gaps": [{"description": "what is missing/wrong and why it matters", "severity": "Clinical/Legal|CQC Inspection|Patient Safety", "section": "where it should be addressed"}],
...
---POLICY DOCUMENT END---`;
    ...
    // Detect Notewell origin via flag OR document text (footer marker in exported DOCX)
    const isNotewell = policy_source === 'notewell' || /generated\s+by\s+notewell/i.test(documentText);
