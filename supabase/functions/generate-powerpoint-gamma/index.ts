...
serve(async (req) => {
  ...
  try {
    ...
    additionalInstructions += ` SPEAKER NOTES: Every slide MUST include detailed speaker notes (also known as presenter notes or meeting notes). The speaker notes should contain: a) A brief explanation of the key points on the slide, b) Additional context, talking points, or background information that would help a presenter deliver the content, c) Any relevant details from the source content that support the slide but are too detailed for the slide itself. Write the speaker notes in full sentences using British English, as if briefing a colleague who will present this material.`;
