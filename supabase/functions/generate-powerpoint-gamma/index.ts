...
serve(async (req) => {
  ...
  try {
    ...
    // Speaker notes / presenter notes - MUST go in PowerPoint's Notes pane, NOT on the slide
    additionalInstructions += ` SPEAKER NOTES (CRITICAL): Every slide MUST have content in the PowerPoint NOTES PANE (the presenter notes section below the slide in PowerPoint). Do NOT place speaker notes or presenter notes as visible text on the slide itself. The notes must ONLY appear in the PowerPoint Notes field — the hidden notes area that presenters see in Presenter View. The slide content should be concise bullet points and visuals only. The Notes pane should contain: a) Talking points and additional context for the presenter, b) Background information that supports the slide content, c) Any relevant details from the source material that are too detailed for the slide. Write notes in full sentences using British English, as if briefing a colleague who will present this material.`;
