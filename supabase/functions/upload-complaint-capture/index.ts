...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const uniqueFileName = `${session.user_id}/${sessionId}/${timestamp}-${fileName}`;
    
    // Upload to storage
    const { error: uploadError } = await supabase.storage
      .from('complaint-captures')
    ...
    console.log(`Successfully uploaded complaint capture: ${uniqueFileName}`);
