...
serve(async (req) => {
  ...
  try {
    const { content, voiceProvider, voiceId, targetDuration } = await req.json();
    ...
    // Step 1: Generate narrative text using AI
    ...
    const narrativeText = aiData.choices?.[0]?.message?.content || content.substring(0, 1000);
