...
${cleanedTranscript}`;
    ...
    // ─────────────────────────────────────────────────────────────────────
    // DIAGNOSTIC SHORT-CIRCUIT (one-shot, payload-shape inspection only)
    // Triggered by `diagnosticDump: true` in the request body. Builds the
    // exact JSON body that would go to Anthropic, dumps the leading and
    // trailing bytes in hex, calls Anthropic's count_tokens endpoint for a
    // real token count, and returns without invoking any model. Removes
    // entirely once the payload-trigger investigation closes.
    // ─────────────────────────────────────────────────────────────────────
    if (diagnosticDump === true) {
      try {
        const claudeModel = modelOverride === 'sonnet-4.6' ? 'claude-sonnet-4-6' : modelOverride;
        const anthropicBody = {
          model: claudeModel,
          max_tokens: 8000,
          system: systemPrompt,
        ...
        const head = bodyBytes.slice(0, 200);
        const tail = bodyBytes.slice(Math.max(0, bodyBytes.length - 200));
        const headHex = toHex(head);
        const tailHex = toHex(tail);
        const headAscii = new TextDecoder().decode(head).replace(/[^\x20-\x7E]/g, '·');
        const tailAscii = new TextDecoder().decode(tail).replace(/[^\x20-\x7E]/g, '·');

        // Scan whole body for suspicious code points.
        let zeroWidth = 0, bom = 0, controlNonWS = 0, replacement = 0;
        for (let i = 0; i < bodyStr.length; i++) {
          const cp = bodyStr.codePointAt(i)!;
          if (cp === 0x200B || cp === 0x200C || cp === 0x200D || cp === 0x2060) zeroWidth++;
          else if (cp === 0xFEFF) bom++;
          else if (cp === 0xFFFD) replacement++;
          else if (cp < 0x20 && cp !== 0x09 && cp !== 0x0A && cp !== 0x0D) controlNonWS++;
        }

        // Anthropic count_tokens
        const anthropicApiKey = Deno.env.get('ANTHROPIC_API_KEY') || Deno.env.get('CLAUDE_API_KEY');
        let realTokenCount: number | string = 'unavailable';
        let countTokensStatus: number | string = 'n/a';
        if (anthropicApiKey) {
          try {
            const ct = await fetch('https://api.anthropic.com/v1/messages/count_tokens', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
            ...
            countTokensStatus = ct.status;
            const ctBody = await ct.text();
            try {
              const parsed = JSON.parse(ctBody);
              realTokenCount = parsed.input_tokens ?? ctBody.substring(0, 200);
            } catch {
              realTokenCount = ctBody.substring(0, 200);
            }
          } catch (e) {
            realTokenCount = `count_tokens_error: ${(e as Error).message}`;
          }
        }
        ...
        const dump = {
          ...
          model: claudeModel,
          payload: {
            total_body_bytes: bodyBytes.length,
            system_chars: systemPrompt.length,
            user_chars: userPrompt.length,
            transcript_chars: cleanedTranscript.length,
            max_tokens: 8000,
            messages_array_length: 1,
            roles: ['user'],
            stop_sequences: 'NOT_SENT',
            temperature: 'NOT_SENT (default)',
          },
          char_audit: {
            zero_width_chars: zeroWidth,
            byte_order_marks: bom,
            unicode_replacement_chars: replacement,
            control_chars_non_whitespace: controlNonWS,
          },
          tokens: {
            real_input_tokens: realTokenCount,
            count_tokens_status: countTokensStatus,
            char_estimate_tokens: Math.round(bodyBytes.length / 4),
          },
          head_200_bytes: { hex: headHex, ascii: headAscii },
          tail_200_bytes: { hex: tailHex, ascii: tailAscii },
        };
        console.log('🔬 DIAGNOSTIC_DUMP ' + JSON.stringify(dump));
        return new Response(JSON.stringify({ ok: true, diagnostic: dump }), {
          status: 200,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      } catch (diagErr) {
        console.error('🔬 DIAGNOSTIC_DUMP_ERROR', diagErr);
        return new Response(JSON.stringify({ ok: false, error: (diagErr as Error).message }), {
          status: 500,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }
