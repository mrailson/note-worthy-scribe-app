...
interface TransformRequest {
  meetingId: string;
  lineContent: string;
  action: 'regenerate' | 'check_tone' | 'reduce' | 'expand' | 'formalise';
  context?: string;
}
...
serve(async (req) => {
  ...
  try {
    ...
    const { lineContent, action, context } = await req.json() as TransformRequest;
    ...
    let systemPrompt = `You are an expert editor for professional meeting notes. You help improve text while maintaining clarity and professionalism. Always respond with ONLY the improved text, no explanations or quotes around it.`;
